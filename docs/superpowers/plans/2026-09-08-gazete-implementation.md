...
- [ ] **Step 3: Write the failing test — `apps/web/src/lib/token.test.ts`**
...
- [ ] **Step 5: Implement `apps/web/src/lib/token.ts`**
...
git commit -m "chore(web): scaffold Next.js app and add token utility"
```
...
**Interfaces:**
- Produces: `sendWelcomeEmail(to: string, preferencesToken: string): Promise<void>`, used by Task 7's subscribe route. Also used later by the worker's digest sender pattern (Task 20 reimplements its own send call in `apps/worker`, since the two apps don't share a runtime dependency on each other — only on `@gazete/db`).
