...
3. Create a `.env` file there (not committed) with: `POSTGRES_PASSWORD`, `RESEND_API_KEY`,
   ...
   `ANTHROPIC_API_KEY`, `VOYAGE_API_KEY` (Voyage AI — powers the free-text "Senin İçin" personalized-interest
   matching in both `web` and `worker`; get a key at dashboard.voyageai.com), `BASE_URL=https://turkiyeningazetesi.com`.
